                    ...
                    <tbody>
                        @foreach ($table as $t)
                            <tr>
                                ...
                                <td class="text-end">
                                    <form action="{{ url('topup-pangan/' . $t->id) }}" method="POST">
                                        <button type="button" data-id="{{ $t->id }}"
                                            data-diamon="{{ $t->diamon }}" data-pangan="{{ $t->pangan }}"
                ...
                <div class="modal-body">
                    <div class="border p-4 rounded">
                        ...
                        <form action="" method="POST" id="formAdd">
                            @csrf
                            <div class="row mb-3">
                                <label for="customerno" class="col-sm-3 col-form-label">Pangan</label>
                                <div class="col-sm-9">
                                    <input type="number"
                                        class="form-control @error('pangan')
                                        is-invalid
                                        @enderror"
                                        id="pangan" name="pangan" placeholder="Enter pangan"
                                        value="{{ old('pangan') }}">
                                    @error('price')
                    ...
                    <div class="border p-4 rounded">
                        ...
                        <form action="" method="POST" id="formEdt">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <input type="number"
                                        class="form-control pangan @error('pangan')
                                        is-invalid
                                    @enderror"
                                        id="pangan" name="pangan" placeholder="Enter pangan"
                                        value="{{ old('pangan') }}">
        ...
        $(document).ready(function() {
            $('.editBtn').on('click', function(e) {
                const id = $(this).data('id'),
                    pangan = $(this).data('pangan'),
                    diamon = $(this).data('diamon'),
                    status = $(this).data('status');
                $('#edtModal').modal('show');
                $('#formEdt').attr('action', "{{ url('/topup-pangan') }}" + "/" + id)
                $('.pangan').val(pangan)
                $('.diamon').val(diamon)
                $('.status').val(status)
            })
